...
public class DocumentController {
     ...
     * 문서 삭제 (Soft Delete).
     *
     * @param fileKey 문서 고유 식별자
     * @return 204 No Content
     */
    @DeleteMapping("/{fileKey}")
    public ResponseEntity<Void> deleteDocument(@PathVariable String fileKey) {
    ...
    public ResponseEntity<EditorConfigResponse> getEditorConfig(@PathVariable String fileKey) {
        log.info("Fetching editor config for document fileKey: {}", fileKey);

        // fileKey로 문서 존재 여부 확인
        if (documentService.findByFileKey(fileKey).isEmpty()) {
